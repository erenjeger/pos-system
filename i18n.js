(() => {
  ...
  const dict = {
    'Dashboard':'Dashboard','POS':'POS','Produk':'Products','Transaksi':'Transactions','PRD':'PRD','Admin':'Admin','Administrator':'Administrator',
    'Point of Sale':'Point of Sale','POS System':'POS System','Bones App':'Bones App','Toko Online':'Online Store',
  ...
  function dynamic(value) {
    ...
    if (lang === 'en') {
      ...
      s = s.replace(/^Stock:\s*(\d+)$/i, 'Stock: $1');
  ...
  function translateAlert(message) {
    if (lang === 'en') return dict[message] || dynamic(message);
    return reverse[message] || dynamic(message);
  }
  const nativeAlert = window.alert.bind(window);
  window.alert = message => nativeAlert(translateAlert(String(message)));
  ...
  function renderLanguage() {
    ...
    addToggle();
    document.querySelectorAll('#bones-lang button').forEach(button => button.classList.toggle('active', button.dataset.lang === lang));
