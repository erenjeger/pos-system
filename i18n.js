(() => {
  ...
  const dict = {
    ...
    'Point of Sale':'Point of Sale','Kelola produk, harga, kategori, SKU, dan stok.':'Manage products, prices, categories, SKUs, and stock.',
  ...
  const tr = s => !s ? s : (lang === 'en' ? (dict[s] || s) : (reverse[s] || s));

  function translateNode(node) {
    if (node.nodeType === 3) {
      const raw = node.nodeValue, trimmed = raw.trim();
      if (dict[trimmed] || reverse[trimmed]) node.nodeValue = raw.replace(trimmed, tr(trimmed));
      return;
    }
    if (node.nodeType !== 1 || node.id === 'bones-lang' || node.closest('#bones-lang')) return;
    ['placeholder','title','aria-label'].forEach(attr => {
      const value = node.getAttribute(attr);
      if (value && (dict[value] || reverse[value])) node.setAttribute(attr, tr(value));
    });
